...
# Use cat to concatenate all .tsv files into a single file with an empty line between each file
for file in *.tsv; do
    # Exclude the output file from concatenation
    if [ "$file" != "$output_file" ]; then
        cat "$file"
        echo   # Add an empty line between each file
    fi
done >> "$output_file"
